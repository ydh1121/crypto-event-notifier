...
setlocal
cd /d "%~dp0"
title CRYPTO COLLECTION @BUILD@
echo CRYPTO COLLECTION @BUILD@
...
1. 기존 프로젝트 폴더 밖(예: 다운로드 폴더)에 압축을 풉니다.
   새로 생성된 버전 폴더를 사용합니다. 이전 폴더에 덮어쓰지 마세요.
...
이번 버전은 기존 원시 체결을 정리하기 전에 이벤트 직전 가격과 15분·1시간·4시간·1일 가격을
별도 보존 테이블에 저장합니다. 실제로 수집된 가격만 보존하며 기존 체결 원장은 수정하지 않습니다.
보존 테이블은 백업 검증 후 수집기가 시작될 때 추가됩니다.
서버가 꺼져 있던 시간이나 이벤트를 늦게 알게 되어 이미 사라진 가격은 만들어 채우지 않습니다.
...
def build(destination: Path):
    ...
    files["RUN_RECOVERY.cmd"] = LAUNCHER.replace('@BUILD@',head[:12]).replace("\n", "\r\n").encode("ascii")
    files["README.txt"] = README.replace('@BUILD@',head[:12]).encode("utf-8-sig")
    ...
    with ZipFile(destination, "w", ZIP_DEFLATED) as archive:
        for name, body in sorted(files.items()):
            archive.writestr("CRYPTO_PAPER_RECOVERY_" + head[:12] + "/" + name, body)
