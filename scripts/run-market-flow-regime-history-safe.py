...
def run_once() -> dict:
    ...
    return {
        ...
        "status": "flow_regime_history_manual_qa_complete",
        "shared_research_work_lock": True,
        "network_fetches": False,
...
def main() -> None:
    payload = run_once()
    print("=== MARKET FLOW REGIME HISTORY LOCKED QA ===")
    print(json.dumps(payload, ensure_ascii=False, indent=2))
    if not payload.get("ok"):
        print("MARKET_FLOW_REGIME_HISTORY_LOCKED=FAIL")
        raise SystemExit(1)
    if payload.get("status") == "deferred_research_work_lock_busy":
        print("MARKET_FLOW_REGIME_HISTORY_LOCKED=DEFERRED")
        raise SystemExit(75)
    print("MARKET_FLOW_REGIME_HISTORY_LOCKED=PASS")
