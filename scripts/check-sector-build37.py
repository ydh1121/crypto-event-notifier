...
def main() -> None:
    ...
    checks = {
        ...
        "strict_manual_header_guard": all(token in identity_safe for token in ("_manual_header", "_manual_identity_matches", "strict_header_and_intro", "symbol_hit", "korean_hit", "english_hit")),
        "strict_manual_split_marker_guard": "가\\s*상\\s*자\\s*산\\s*소\\s*개" in identity_safe and "if not positions" in identity_safe,
