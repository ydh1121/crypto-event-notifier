...
| F04 | P0 | 실제 완결 표본 확인 대기 | 이벤트 기준가·목표가 확보 | 새 PC 보고의 단일 SEC 이벤트는 BTC/ETH 4개 구간 완료, B3 기준가 결측. 직전 수집의 2,813회 도래 시도 중 기준가 2,344·목표가 469 누락. 시도와 이벤트 수 구분. | 새 실행본의 전체 표시 이벤트 목록에서 실제 코인/BTC/ETH 완결 표본을 대조하고, 없으면 새 발표의 기준가부터 누적 확인. |
...
| F11 | P1 | 뉴스·지표 비교 표 구현·나머지 보완 | 가상매매 코인 UI·반응도 | 4145ba3 실행본에 발표별 수익률·BTC/ETH 대비·표본 표와 4개 시간 선택, 실제 가격/시각 상세 연결. 첨부 실제 반응값 대조 완료. | 실제 화면 적용 확인, 코인 고정 전략/반응도와 5개 세부 탭 통합. 미수집 범위는 완료 처리하지 않음. |
...
| F30 | P1 | 재분류 | 기존 기타 기능 보존·배치 | 기존 Assets, 전략 연구, 시장/테마, 기록, 운영 기능을 재사용. | 연구·시장과 설정·운영으로 역할 배치. 정상 기능 삭제나 숨긴 뒤 접근 불가 상태 금지. |
...
최신 제공 실행본은 4145ba3 / artifact version 10이며 추출 검증을 마쳤다. 실제 PC에서 마지막으로 확인된 조회 실행본은 e392f22 / version 9이다. 실제 계획 저장/복원·새 화면 적용·디자인 수용은 별도 확인한다. 다음은 F04의 실제 코인/BTC/ETH 완결 표본 대조이며 TASKS.md와 HANDOFF.md의 현재 항목을 따른다.
