...
| F08 | P1 | 소스 검증 완료·실행본 대기 | 계획 영속 저장 | 작업 트리에 계획 버전·동시수정 충돌·중복요청 방지 구현. 아직 새 실행본 미제공. | 기존 별도 보유 DB 첫 저장 전 백업·검증, 재시작 복원, 추출 패키지 검증 후 전달. |
| F09 | P1 | 소스 검증 완료·실행본 대기 | 수동 소액 체결·PAPER 비교 | 작업 트리에 수수료·분할매도·취소 이력·같은 코인/전략/기간 비교 구현. 실제 PC 적용 전. | 계획 저장→실제 체결 입력→원장→완료 거래 비교를 검증. 매도 완료 자산의 기록 재접근 source 검증 완료; 실제 PC 저장·복원 확인 대기. |
...
1. **현재 단위:** 계획 저장·수동 소액 기록과 매도 완료 기록 재접근 구현을 마쳤다. 이어서 추출 실행본에서 저장→재시작 복원→분할 체결→같은 코인/전략/기간 비교를 검증하여 제공한다.
...
현재 실제 PC 적용본은 a330b8b / artifact version 8이다. 코딩 환경 연결을 복구했고 계획 저장·수동 기록·매도 완료 기록 재접근의 source 검증을 마쳤다. 새 실행본 추출 검증·전달과 실제 PC 적용은 아직 남아 있다. 상세 복귀점은 TASKS.md와 HANDOFF.md를 따른다.
