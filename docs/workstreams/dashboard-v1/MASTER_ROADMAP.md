...
## 현행 제품 목표·우선순위·기능 체크 — 2026-09-26 KST

이 절은 최신 사용자 요구를 반영한다. 아래의 과거 Build/Phase 완료 표시는 보존하되 현재 기능의 종단 완료 판단은 이 절의 확인 범위를 따른다. UI 승인 선행 또는 실패한 V22를 디자인 정본으로 삼는 과거 지시는 적용하지 않는다.

1. 코인별 BTC/ETH, 대내외 뉴스·지표 반응을 로컬에 지속 누적하고 코인별 거래 로직·실시간 평가·개선 알고리즘을 확립한다.
2. 거래소×코인×전략 독립 PAPER 계좌, 차트 타점, 실제 원장으로 해당 코인의 전략 성과를 검증한다.
3. 실제 보유자산과 전략별 분할 진입/익절/물타기 계산, 계획 저장, 수동 소액 실전 기록을 연결한다.
4. 사전 검증 기준을 통과하면 승인된 실전 범위에 적용하고 결과를 다시 평가·개선에 반영한다.

**등급:** P0 누적·정합성 복구 / P1 사용자 매매 흐름·UI / P2 반응도 소스 확장 / P3 검증 기반 추천·개선·실전 전환. 선행조건에 따른 순서이며 낮은 등급이 목적상 불필요하다는 뜻은 아니다.
**상태:** source 존재·테스트 통과·PC 적용·사용자 UI 승인을 구분한다. ‘범위 한정 완료’는 적힌 실제 관측 범위만 완료이고 전체 코인/기간으로 일반화하지 않는다. ‘미확인’은 부재를 단정하지 않는다.

| ID | 등급 | 상태 | 기능 | 확인 범위 | 다음 완료 조건 |
|---|---|---|---|---|---|
| F01 | P0 | 부분 확인·확장 | 코인별 가격·체결·시장 메모리 | 실제 PC에서 PAPER·메모리와 빗썸/업비트 BTC·ETH 체결 흐름 증가 확인. 전체 코인·장시간 연속성은 미확인. | 코인별 최신시각·수집 간격·누락구간을 T0/T1로 대조. |
| F02 | P0 | 부분 확인·수정 | OHLCV 장기 누적 | collector와 테이블 존재, 직전 16시장 저장 확인. source 기본 400봉 보존은 장기 연구 요구와 별도 검토 필요. | 원천 장기보관·보관주기·재집계 정책을 정하고 기존 데이터 삭제 없이 적용. |
| F03 | P0 | 범위 한정 완료 | 코인×전략 PAPER 계좌·체결 원장 | B3/빗썸/공격적 8체결·3완료·2승, 계좌와 원장 대조 확인. 전체 코인 검증 완료나 전략 우위의 증거는 아님. | 타 거래소·코인·전략 표본으로 대조 범위를 확장. |
| F04 | P0 | 실제 완결 표본 확인 대기 | 이벤트 기준가·목표가 확보 | 4145ba3 실제 보고의 표시 이벤트 4건 모두 B3 기준가 결측. 코인/BTC/ETH 완결 표본 0건. | 새 이벤트 이전의 기준가 확보부터 15m/1h/4h/1d 누적을 연결. 과거 결측을 이후 가격으로 대체하지 않음. |
| F05 | P1 | 연결됨·보완 | BTC·ETH 상대 움직임 | market memory, market_relative_strength와 코인 화면 경로 존재. BTC 도미넌스 데이터는 별개. | 동일 거래소·동일 시간창 수익률, 초과수익률, 표본·시각을 반응도 표로 연결. |
| F06 | P1 | 범위 한정 완료 | 실제 보유자산·빗썸 지정·원화 평가 | 사용자가 모든 보유분을 빗썸으로 확정. 실제 화면 4종목 원화 평가 확인. ETH 개당 상세와 공개 가격 새로고침 성공은 미확인. | ETH/BTC 원화 환산 근거·갱신시각 확인. 취득시점 환율 없는 원화 손익은 미확인 유지. |
| F07 | P1 | 연결됨·보완 | 전략별 분할 진입·비중·익절·계산기 | 기존 수수료 계산을 재사용하며 실제 보유량·평단과 해당 코인 PAPER 전략을 연결. | 전략 탭 전환, 매수/익절 회차, 예상 평단·수수료·손익·잔량을 한 흐름으로 검증. |
| F08 | P1 | 실행본 검증 완료·PC 적용 확인 대기 | 계획 영속 저장 | e392f22 실행본 version 9 제공. 최초 저장 백업·재시작 복원·계획 버전·동시수정 충돌·중복요청 방지 추출 실행본에서 검증. | 실제 PC에서 기존 원화 계획 저장·복원 확인. 다른 통화의 계획 저장은 별도 범위. |
| F09 | P1 | 실행본 검증 완료·PC 적용 확인 대기 | 수동 소액 체결·PAPER 비교 | 수수료·분할매도·취소 이력·같은 코인/전략/기간 비교, 매도 완료 자산 기록 재접근까지 실행본 검증 완료. | 실제 PC 적용 확인 후 실제 소액 체결을 누적·대조. 테스트 표본을 실전 성과로 간주하지 않음. |
| F10 | P1 | 수정 | 실전매매 UI | 기존 Assets 기능을 보존하고 보유자산→코인→전략→분할 계획→계산 순서로 통합 필요. | 첫 화면은 현재가·보유 손익·선택 전략·가격/비중. 계획과 소액 매매는 별도 탭. |
| F11 | P1 | 뉴스·지표 비교 표 연결·나머지 보완 | 가상매매 코인 UI·반응도 | 4145ba3 실제 조회 실행과 4건의 이벤트 목록 확인. 숫자 표·기간 선택·가격/시각 상세 구현, 최종 시각 수용 미확인. | 코인 고정 전략/반응도와 5개 세부 탭 통합. 미수집 범위는 완료 처리하지 않음. |
| F12 | P2 | 추가 구축 | BTC 도미넌스·OTHERS | 현재 source 조사에서 코인별 반응 누적의 완성 경로를 확인하지 못함. | 출처·분모/제외종목·구성 변경 이력을 보존한 시계열과 코인 반응을 연결. |
| F13 | P2 | 추가 구축 | SNS·검색량·과열 심리 | 검색량, 긍정/부정 단어, 대중 과열 지표의 지속 누적·검증 경로 미확인. | 시각·언어·플랫폼·표본·봇/중복·기준기간을 저장. 과열 지표를 바로 매도 명령으로 사용하지 않음. |
| F14 | P2 | 추가 구축 | 온체인 반응 | 고래 이동·등급별 지갑 활성·거래소 대량 이동의 완성 경로 미확인. | 체인/토큰 식별, 지갑 등급 기준, 주소 출처·신뢰도, 블록시각, 입출금 구분과 코인 반응 연결. |
| F15 | P2 | 기반 있음·확장 | 지정학·전쟁·돌발 뉴스 | SEC/CFTC 공식 뉴스 기반은 있으나 전쟁·돌발 뉴스 전체 범위를 대신하지 못함. | 뉴스 원문·최초 관측·발표시각·대상 코인·직접 관련/시장 공통 분류·중복 이벤트를 보존. |
| F16 | P2 | 기반 있음·연결 확인 | FOMC 금리 결정·회의록 | 공식 일정 어댑터 및 이벤트 종류 존재. 날짜만 있는 회의 일정은 발표시각/실제 금리 결정이 아님. | 금리 결정과 회의록을 각각 실제 발표시각·결과·당시 예상치·반응에 연결. |
| F17 | P2 | 기반 있음·운영 확인 | CPI | BLS 일정·actual 캡처 source 존재. 코인별 누적 충분성 미확인. | 최초 발표값·직전값·발표 전 예상치·개정값을 구분하고 반응 표 연결. |
| F18 | P2 | 기반 있음·운영 확인 | PCE | BEA 일정·actual 관련 source 존재. 실제 데이터와 코인 반응 완주 미확인. | PCE 종류·단위·발표 차수·발표시각별 actual/예상/반응 검증. |
| F19 | P2 | 일정 기반·확장 | PPI | BLS 일정 등록 존재. actual·예상치·코인별 반응 완성은 미확인. | 지표별 actual 수집과 발표 시점 반응 연결. |
| F20 | P2 | 기반 있음·운영 확인 | 미국 비농업 고용·실업률 | BLS 고용 actual 및 비농업 증감/실업률 계산 source 존재. | 한 보고서 안의 두 지표를 구분하고 최초 발표 당시 값으로 반응 계산. |
| F21 | P2 | 추가 구축 | 미국 신규 실업수당 청구 | 현재 완성 수집·반응 경로 미확인. | 주간 발표·수정치·예상치·발표시각과 코인별 반응 누적. |
| F22 | P2 | 추가 구축 | 미국 국채 2년물·10년물 / DXY | 요청 범위의 연속 관측·코인 반응 경로 미확인. | 수익률 변화와 지수 수익률을 구분하고 코인과 동시각 창 정렬. |
| F23 | P2 | 기반 있음·연결 확인 | 미국 GDP | BEA 일정·actual 기반 존재. 발표 차수별 코인 반응 완주 미확인. | 속보·잠정·확정 등 발표 차수와 당시 이용 가능 값 구분. |
| F24 | P2 | 추가 구축 | 제조업·서비스업 PMI | 현재 요청 지표의 완성 수집·반응 경로 미확인. | 기관·제조/서비스·발표 차수를 구분하여 수집. |
| F25 | P2 | 수정·확장 | NASDAQ 100 / S&P 500 | 현재 미국 지수 source는 NASDAQ_COMPOSITE, SP500, VIX. 종합지수는 NASDAQ 100 대체값이 아님. | 나스닥 100을 독립 시리즈로 추가하고 거래시간·지연 여부·동시각 반응 검증. |
| F26 | P2 | 추가 구축 | 미시간대 소비자심리지수 | 완성 수집·반응 경로 미확인. | 예비/확정 발표·실제값·당시 예상치·코인 반응을 구분. |
| F27 | P2 | 기반 보존·확장 | 거래소 공지·상장·유의·거래지원 종료 | 기존 거래소 공지/상장 수집과 연구 기능 보존. 전체 코인 반응 완주는 별도 검증. | 실제 발표/효력 시각, 거래소와 해당 시장을 연결하고 주요 반응도에 통합. |
| F28 | P3 | 기반 있음·확장 | 코인별 거래 로직·실시간 평가·개선 알고리즘 | profiles/feedback/strategy lab 학습 및 shadow readiness 기반 존재. 새 반응 변수의 유효성은 미검증. | 버전 고정→시간 순 학습/검증 분리→비용 반영 PAPER 비교→열화 감지→승격/롤백. 새 변수는 우선 shadow. |
| F29 | P3 | 재설계 | 상승 가능성 높은 코인 추천 | 기존 후보 점수만으로 요청한 근거 기반 추천이 완료된 것은 아님. | 시장구조·코인별 전략 표본·BTC/ETH·이벤트·과거 동일 상황을 연결. 표본 부족 시 관찰 후보, 확률 생성 금지. |
| F30 | P1 | 고정 실행 경로 구현·나머지 재분류 | 기존 기타 기능 보존·배치 | CRYPTO 고정 폴더, 수집·조회·점검 실행 구분, 검증된 구버전 정리·결과 보존을 ba0c17f로 제공. 기존 Assets·연구·시장·기록 기능 보존. | 실제 PC에서 새 런처/정리 적용 확인. 연구·시장과 설정·운영으로 나머지 기능 배치. |
| F31 | P3 | 검증 후 별도 승인 | 소액 성공 판정·실전 전환 | 수익 여부만으로 성공/자동 승격을 판정할 수 없음. 실주문 활성화 미승인. | 표본·기간·비용·낙폭·손익 집중·체결 차이를 사전 기준으로 대조하고 실전 전환은 별도 명시 승인. |

### 반응도 데이터·개선 알고리즘 계약

- 거래소+코인+통화, 출처와 시리즈 정의, 발생/발표/블록/체결시각, 최초 수신시각, 개정 차수, 실제값·발표 전 예상치·이전값을 구분한다. 과거 시점에 아직 없던 값을 학습·추천에 섞지 않는다.
- 동일 시간창의 코인/BTC/ETH 기준가·목표가와 실제 시각으로 15m/1h/4h/1d 반응을 누적한다. 미도래·결측·수집 실패·표본 부족은 0%와 다르다.
- 연속 지표(도미넌스·OTHERS·채권금리·DXY·주가지수)와 발표 이벤트는 시간 기준을 분리한다. 도미넌스 분모와 OTHERS 구성 변경, SNS 플랫폼/언어/봇/중복, 온체인 체인/토큰/주소 출처·등급 버전을 보존한다.
- 뉴스는 직접 관련/시장 공통/연관 불명과 중복 사건을 구분한다. 우연한 동시 움직임을 인과관계로 단정하지 않는다.
- 최초 발표값·개정값·나중에 확보한 예상치를 분리한다. NASDAQ_COMPOSITE를 NASDAQ 100으로 이름만 바꾸지 않는다.
- 코인별 규칙 후보는 버전·학습기간·검증기간을 고정하고 비용·유동성·시장상태를 반영하여 시간 순 검증과 PAPER A/B를 거친다. 실시간 재평가와 자동 규칙 승격은 별개다. 새 반응 변수는 우선 shadow에서 평가하며 원래 전략을 조용히 덮어쓰지 않는다.
- 추천은 시장구조·코인별 전략 표본·BTC/ETH·최근/동일 유형 이벤트와 연결한다. 검증·보정되지 않은 상승 확률을 생성하지 않는다. 표본 부족은 관찰 후보로 표시한다.
- 소액 성공 판정에는 표본·기간·비용·낙폭·손익 집중·체결 차이를 사전에 정한 기준으로 비교한다. 실주문 활성화·Production 배포·기존 전략/PAPER 의미 변경은 별도 명시 승인 경계를 유지한다.

### 화면별 기능 소유권

| 화면 | 주 흐름 | 핵심 구성 |
|---|---|---|
| 실전매매 | 보유 목록 → 코인 → 전략 → 계획/소액 기록 | 거래소·티커·현재가·내 수량/평단/손익, 전략별 분할 진입가/비중·익절가/비중, 물타기/익절 계산, 수수료·예상 평단·손익·잔량, 계획 저장/복원 |
| 가상매매 | 코인 고정 → 전략 / 반응도 | 전략별 독립 계좌·차트 타점·체결 원장·성과·진입/청산 조건 |
| 반응도 | 시장 비교 / SNS·검색 / 온체인 / 뉴스·지정학 / 거시지표 | 숫자 중심 4~5열 표와 기간 전환, 선택 행에서 네 반응창 비교, 원문·산식·가격/시각은 상세 |
| 상승 후보 | 후보 → 해당 코인의 검증 근거 | 해당 코인의 전략·시장구조·상대 움직임·이벤트·표본. 부족한 근거를 확률로 위장하지 않음 |
| 연구·운영 | 시장/연구 / 기록 / 설정·운영 | 기존 시장·테마·전략 전체 비교·DEX/상장·프로젝트 평가, 체결/판단/보유이력, 수집·연결·알림·백업·서버 상태 |

- 모든 실제 보유분은 사용자 확정대로 빗썸. ETH/BTC 원화 환산 단가와 보유 총 평가액을 구분하고 취득시점 환율 없는 KRW 손익을 만들지 않는다.
- 코인별 성과와 전략 전체 성과를 섞지 않는다. 수익률 1등은 검증된 우위와 다르다.
- 핵심 숫자는 2~3초 안에 읽힌다. 장문 표, 카드 안 카드, 장식 KPI, 반복 상태 설명, 작은 회색 글씨, 내부 개발 용어를 주 화면에 놓지 않는다.
- 입력·선택·펼침·스크롤은 갱신 후에도 유지한다. 종목 매도 완료 뒤에도 계획·수동 기록 재접근을 보존한다.
- 기존 정상 기능은 새 역할로 배치하고 연결한다. 메뉴 변경/CSS 패치/정적 검사 통과만으로 UI 완성 처리하지 않는다.

### 순차 실행과 현재 복귀점

1. **완료한 구현 단위:** 계획 저장·수동 소액 기록·매도 완료 기록 재접근을 e392f22 실행본 version 9로 제공했다. 추출 실행본의 저장→재시작 복원→분할 체결→같은 코인/전략/기간 비교와 기존 DB 보존을 검증했다. 실제 PC 적용은 별도 확인 대기다.
2. **다음 P0:** 새 실제 이벤트 한 건의 코인/BTC/ETH 기준가와 15m/1h/4h/1d를 끝까지 연결한다. 과거 기준가를 보간해 실제값으로 표시하지 않는다.
3. **다음 P1:** 확보된 데이터를 반응도 세부 탭·숫자 표에 연결하고 실전매매의 전략별 계획 흐름을 통합한다. 한 질문의 DB→API→화면을 완료한다.
4. **P2:** 기존 거시·공지·미국 지수 경로를 먼저 완주한다. 도미넌스/OTHERS → SNS → 온체인 → 지정학/신규 거시 시리즈를 차례로 확장한다.
5. **P3:** 코인별 로직 버전·상승 후보를 검증하고 소액 실전 결과를 환류한다. 별도 승인 없는 자동 실전 전환은 하지 않는다.

최신 제공 실행본은 ba0c17f / artifact version 11의 CRYPTO.zip이다. 앞으로 바탕화면 CRYPTO 한 폴더에 덮어쓰며 버전 폴더를 늘리지 않는다. 수집은 START_COLLECTION.cmd, 조회는 RUN_REVIEW.cmd, 상태 확인은 RUN_CHECK.cmd, 검증된 구버전 정리는 CLEAN_OLD_FOLDERS.cmd로 고정한다. 본체 crypto-event-notifier-live는 코드·DB·계획·백업을 보관하므로 삭제하지 않는다. 실행 중인 구버전 복구 폴더는 보존하고 결과 파일을 옮긴 뒤 검증된 도구만 정리한다. 관련 57검사와 실제 구형 ZIP 형식 정리·추출 검증을 통과했으며 실제 PC 정리/새 런처 실행은 미확인이다.

실제 PC 조회 실행은 4145ba3까지 확인했다(9월 26일 14:56–14:57 KST). 표시된 이벤트 4건 모두 B3 기준가가 결측이며 완결 비교 표본은 0건이다. F04는 미완료로 유지하고 새 이벤트 이전 기준가 확보부터 네 반응창까지 누적을 연결한다. 실제 계획 저장/복원·디자인 수용은 별도 확인한다. TASKS.md와 HANDOFF.md의 최신 항목을 따른다.
