...
| F04 | P0 | 수정 | 이벤트 기준가·목표가 확보 | 직전 수집 보고의 도래 시도 8,990 중 기준가 누락 8,400·목표가 누락 590. 이벤트 수와 혼동 금지. | 새 실제 이벤트 한 건의 코인/BTC/ETH 15분·1시간·4시간·1일 연결부터 완결. |
...
| F11 | P1 | 수정 | 가상매매 코인 UI·반응도 | 코인별 전략·BTC/ETH·이벤트 경로 존재. 새 반응도 범위와 장문 표 문제 미해결. | 코인 고정 후 전략/반응도 전환. 반응도 세부 탭과 숫자 중심 표, 원문·산식은 행 상세. |
...
최신 제공 실행본은 e392f22 / artifact version 9이며 추출 실행본 검증을 마쳤다. 마지막으로 확인된 실제 PC 적용본은 a330b8b / version 8이다. 다음 개발은 F04/P0 새 실제 이벤트의 코인/BTC/ETH 반응 연결이며, 새 실행본의 실제 PC 적용과 디자인 수용 여부는 별도 확인한다. 상세 복귀점은 TASKS.md와 HANDOFF.md를 따른다.
