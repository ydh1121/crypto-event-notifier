...
| F08 | P1 | 구현·검증 진행 | 계획 영속 저장 | 작업 트리에 계획 버전·동시수정 충돌·중복요청 방지 구현. 아직 새 실행본 미제공. | 기존 별도 보유 DB 첫 저장 전 백업·검증, 재시작 복원, 추출 패키지 검증 후 전달. |
| F09 | P1 | 구현·검증 진행 | 수동 소액 체결·PAPER 비교 | 작업 트리에 수수료·분할매도·취소 이력·같은 코인/전략/기간 비교 구현. 실제 PC 적용 전. | 계획 저장→실제 체결 입력→원장→완료 거래 비교를 검증. 매도 완료 자산의 기록 재접근 보완. |
...
1. **현재 단위:** 미커밋 계획 저장·수동 소액 기록을 보존한다. 실행 환경 연결 복구 후 남은 기록 재접근 경로를 확인하고, 추출 실행본에서 저장→재시작 복원→분할 체결→같은 코인/전략/기간 비교를 검증하여 제공한다.
...
현재 실제 PC 적용본은 a330b8b / artifact version 8이다. 계획 저장·수동 기록은 작업 트리 구현과 테스트까지 진행했으나 실행 환경이 environment_offline 상태가 되어 source commit/새 ZIP/실제 PC 적용은 아직 완료되지 않았다. 상세 복귀점은 TASKS.md와 HANDOFF.md를 따른다.
