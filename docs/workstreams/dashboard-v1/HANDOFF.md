...
### Active — manual planning and small-real-trade records ready for package verification

- **State: SOURCE_VERIFIED / PACKAGE_VERIFICATION_NEXT.** Coding environment reconnected; the original dirty source was preserved. Local docs fast-forwarded to 77e7e7f with no source overwrite. This unit completes the previously recorded plan/record implementation and zero-holding history access.
...
- Next in this unit: commit source → build with --enable-planning --holdings-exchange bithumb → extracted stdlib Python/HTTP/JS verification of save, server restart, partial fills, closed archive and DB preservation → replace existing Viewer artifact version 8. No new package has been delivered at this source checkpoint.
