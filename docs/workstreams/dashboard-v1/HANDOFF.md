...
### Current — fixed CRYPTO workspace delivered; actual 4145ba3 report verified

- **State: FIXED_WORKSPACE_DELIVERED / ACTUAL_EVENT_INDEX_REVIEWED / F04_COMPLETE_SAMPLE_PENDING.** User rejected version-folder proliferation and authorized one fixed tool folder plus safe old-folder cleanup. This policy supersedes earlier instructions to create a new version directory for each Viewer delivery.
- Actual attachment CRYPTO_B3_REVIEW_RESULT(3).json: 62,341 bytes, SHA256 e4e4d3ac4e63d1ae44989d80d6aface902bc60f7daed90d5bafaac7083a5cbd1. Observation 2026-09-26 14:56:41–14:57:15 KST, not the later submission time. Viewer 4145ba3 integrity verified; PAPER DB 3,935,420,416 bytes. B3/aggressive reconciles 9 fills / 4 closes / 2 wins. Four active / one closed Bithumb holdings remain unchanged. Actual manual save and visual acceptance are not proven.
- PAPER/memory/lab account+metrics and sampled BTC/ETH trade streams advanced; owner identities match. Last OHLCV cycle: 16 markets / 3,985 rows / zero consecutive deferrals. Lab: 188 source rows / zero fills. Intelligence: one eligible event / 775 markets / 287 already captured; 2,813 due attempts = 2,344 missing baselines + 469 missing targets; zero inserts and source/response errors. Counts of attempts are not counts of events.
- The displayed event index contains four events, all missing B3 baselines at all four horizons: zero complete coin/BTC/ETH comparisons. One SEC event has eight benchmark observations; another has six; the other two events lack baselines across all three assets. F04 remains pending. Do not repeatedly recheck only one old event, backfill a baseline with later prices, or claim full event coverage.
- Source ba0c17fdc9301e886fa48deba6595dd4b2a7cd61 delivers constant ZIP root CRYPTO/. START_COLLECTION.cmd runs only the existing clean canonical checkout's local_process_host --recovery with collection-only settings and duplicate/unknown-process guards. It does not fetch, checkout, install, replace DBs or supply collector source. RUN_REVIEW / RUN_CHECK retain their prior behavior and confirmed Bithumb setting.
- Future tool updates: close the Viewer window and overwrite the same Desktop/CRYPTO folder; leave collection running. Canonical crypto-event-notifier-live remains the collector code, Python, SQLite, plans and backups owner and must be kept. A stopped collector is launched only through CRYPTO/START_COLLECTION.cmd. Existing collector source is still last PC-verified 4af291d; no fresh local Git read or Windows startup execution is claimed.
- CLEAN_OLD_FOLDERS.cmd scans only sibling/Desktop/Downloads roots for recognized old review/recovery bundles. It verifies manifests and hashes, rejects unknown files/DBs/links, preserves result JSON in CRYPTO/reports/previous, checks processes before and after rename, and restores on concurrent change. Running old recovery parents retain their package; after that old session ends, the canonical launcher no longer needs the old folder. No forced restart or blind recursive deletion.
- Delivered CRYPTO.zip as version 11 of the existing Viewer artifact: 51 files / 112,578 bytes, SHA256 8566372c521ffa26a90d95fbfcf6a70357068b71fa0cc60dd2261861ac200020; manifest b45c238054a57e1c7ea43f9c8b2d1f510b175e60c63bb88dcadc8cea3cf73c5a. Package version stays inside its manifest, not the folder name.
- Verification: 57 affected tests passed, including shared runtime/host lifecycle; 8 launch/overlay tests rerun after receipt hardening. Two revisions overlay one folder without losing outputs or changing the canonical fixture DB. Extracted stdlib imports and 50 manifest file hashes pass; both previously delivered real ZIP formats clean successfully with their result files preserved. All 22 unchanged JS modules remain included. These are isolated tests, not evidence of deletion/startup on the user's PC.
- **Next:** F04/P0: inspect pre-release B3/BTC/ETH price capture for a fresh event and connect actual 15m/1h/4h/1d observations; then finish reaction sub-tabs and numeric tables. F08/F09 actual manual save/restore and ETH per-unit/public-refresh evidence remain separate gaps. No canonical data mutation, collector restart, rule change, primary merge, production deployment or live order activation occurred.

## Previous checkpoint — actual e392f22 report and versioned event package

- **State: ACTUAL_REVIEW_BUILD_VERIFIED / EVENT_COMPARISON_PACKAGE_VERIFIED / F04_ACTUAL_COMPLETE_SAMPLE_PENDING.** Actual report CRYPTO_B3_REVIEW_RESULT(1).json (55,025 bytes; SHA256 c8ebffef47075c286afd61164698709e12ec53b03e00924b880881eaa14c6747) confirms Viewer e392f22 with valid integrity. Observation: 2026-09-26 14:23:47–14:24:29 KST. This verifies that revision's read path, not an actual manual plan save or browser design acceptance.
- Canonical PAPER path remains b3_trader/data/auto_demo.sqlite3, reported size 3,921,215,488 bytes. B3/aggressive reconciles nine fills / four completed trades / two wins and is flat. The latest ninth fill is a strategy stop exit; no trading rules were changed. Four active / one closed actual holdings remain resolved to Bithumb, with three KRW-plannable and one native BTC record. No private holding amounts or attachment bytes are committed.
- PAPER, market memory, strategy lab accounts/metrics and sampled BTC/ETH trade streams advanced across the observation. Owner process checks match. Last lab cycle processed 130 source rows / zero fills; OHLCV wrote 4,068 rows for 16 markets with zero consecutive deferrals. Last intelligence cycle: one eligible event / 775 markets / 287 already captured; 2,813 due attempts = 2,344 missing baselines + 469 missing targets; zero inserts and zero source/response failures. Attempts are not event counts. Unchanged event clocks in this short observation do not establish a stopped collector.
- Actual event evidence: the report's SEC event has all four BTC and ETH reactions (eight observations), but B3 lacks its baseline at every horizon. B3 target ticks exist for 15m, 1h and 1d; the 4h target is missing. Missing values remain null. This historical baseline cannot be replaced with a later price or an interpolated value.
- Implemented source 4145ba31eee8c02fe4a270ded1bc8281a4d8d9f3: the coin's 뉴스·지표 tab now compares releases in a five-column numeric table with 15m/1h/4h/1d controls. Selecting a row retains exact price/time and historical-sample details. Polling preserves period, event, focus and disclosures. The report now indexes all displayed events (at most 20 with local price evidence) and includes exact evidence for the most recent event plus the best available complete comparison. This is not an inventory of every news item or a recommendation.
- Delivered CRYPTO_STRATEGY_REVIEW_4145ba3.zip as existing Viewer artifact version 10: 46 files / 107,810 bytes; SHA256 72f98318ff75ceb9ea97374c12977c8ae492a09eb92fd54cd2bd8756448aa7fa; manifest SHA256 01904cf060fdce48416f779e9c76fb34c3af5f225ee8e18a4de0ff9059ecdeef. Existing manual planning, owner-confirmed Bithumb and read-only RUN_CHECK are retained. Close only the old review window, extract the new folder, run RUN_REVIEW.cmd; keep collectors running.
- Verification: 20 affected Python and eight coin-view DOM tests passed, TypeScript noEmit / changed Python compile / diff checks passed. Extracted stdlib Python and all 22 JS modules resolve. The attached actual event was passed through extracted Python and JS: all eight benchmark returns agree at displayed precision; all missing B3 returns/relative values remain missing. Separate SQL fixtures prove a complete older comparison remains in the report when the latest event lacks its coin baseline, with the DB hash unchanged. No browser screenshot/visual acceptance or new Windows collection is claimed.
- **Next:** inspect the complete displayed-event index from the new actual report, select a real event with coin/BTC/ETH anchors, and compare all four horizons to the stored rows. If no such event exists, retain F04 pending and inspect accumulation around a fresh release. Then complete the reaction sub-tab hierarchy and expand the documented P2 sources. Do not keep rechecking only the same old missing-baseline event or manufacture a completed sample. Actual manual plan save/restore and ETH per-unit/public-refresh evidence remain separate gaps.
- No collector restart, canonical DB mutation, credential change, PAPER semantics change, primary merge, production deploy or live order activation.

## Previous delivered checkpoint — manual planning package
