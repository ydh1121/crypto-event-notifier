...
1. Keep any currently active collection window open. Use the newly versioned folder's `RUN_CHECK.cmd`; it needs no viewer port, saves `CRYPTO_CHECK_RESULT.json` after about 30 seconds and exits automatically. An existing old review window may remain open. Do not rerun recovery merely to obtain diagnostic evidence.
2. Read the verified reviewer identity, actual collector last_result, event baseline/target/future counters and observed-market count. B3 ledger and OHLCV advancement are now established; do not restart their diagnosis from zero. Investigate missing event reactions using actual outcomes, never the old report's events=0 field.
3. Source retry fixes after c5ffd52 are still not applied on the actual PC. Any later collector update needs a confirmed stopped-process/backup transition; no blind restart. Continue coin/event/BTC/ETH evidence and existing workspace visual review, then real-holdings planning.
