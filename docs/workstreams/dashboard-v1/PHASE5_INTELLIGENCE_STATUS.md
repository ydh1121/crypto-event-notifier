...
5. Define minimum sample, dispersion, recency, provider-quality and regime-stability gates.
6. Produce shadow EventScore / RegimeScore / RelativeStrengthScore contributions only for groups that pass those gates.
7. Run baseline vs intelligence-v2 PAPER A/B without altering the current baseline strategy.
8. Validate walk-forward/out-of-sample behavior before any candidate promotion.
9. Project evidence and explanations into the Viewer only after the backend contracts are stable.

## 7. Current implementation commits
...
- `f6c9e97` bounded BLS CPI/Employment initial-actual capture + ingest-cycle integration
