...
**Status: OWNER_CONFIRMED_BITHUMB / KRW_PRICE_REVIEW_READY. Owner explicitly confirms all actual holdings are Bithumb. Actual PC c85f545 screen/report observed; its public quote request was partial. The new configured Viewer resolves that declared exchange and displays per-unit KRW prices with explicit valuation provenance. PC failure cause and new-version visual acceptance remain unverified.**
...
- Verification: 24 distinct affected Python tests and 11 holdings JS/DOM tests passed; TypeScript, Python compile and diff checks passed. Covered owner-declared identity resolution with unchanged journal hashes and unchanged default behavior, native conversion vs direct same-exchange KRW basis, per-unit vs total values, missing/future/stale/foreign prices, partial HTTP batch failure, retained source clocks, fee-aware split plans and UI continuity. Package verification follows the pinned configured build. Prior localhost-browser policy block was not bypassed.
