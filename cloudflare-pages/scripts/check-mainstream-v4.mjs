...
check('main navigation follows simplified trading hierarchy',index.includes('>실전매매</button>')&&index.includes('>가상매매</button>')&&index.includes('>탐색</button>')&&index.includes('>기록</button>')&&!index.includes('>홈</button>')&&!index.includes('>자산</button>')&&!index.includes('>전략</button>'));
...
check('mobile navigation exposes four primary items in one row',css.includes('grid-template-columns:repeat(4,1fr)!important'));
...
check('decision workspace stylesheet loads after dark audit',/\/modules\/styles\/decision-workspace-v4\.css\?v=[^'\"]+/.test(index)&&index.indexOf('theme-dark-audit-v4.css')<index.indexOf('decision-workspace-v4.css'));
