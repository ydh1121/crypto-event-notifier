...
export function installAmountInputUx(root){
  ...
  const refresh=()=>queue();
  const focusin=event=>{
    if(event.target.matches?.('[data-direct-amount],[data-avg-amount]'))queue();
  };
  const click=event=>{
    ...
    if(event.target.closest('[data-add-avg-row],[data-add-recommended-row],[data-reset-avg],[data-remove-avg]'))queue();
  };
