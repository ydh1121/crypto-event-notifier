const ROUTES=new Set(['dashboard','research','assets','paper','strategy','records','system']);
export function createRouter({store,root,nav,pages}){
  ...
  function go(name,{replace=false}={}){
    ...
    if(currentName===name){current?.render?.();syncNav(name);return}
    current?.destroy?.();currentName=name;store.setUi({route:name},{scope:'router'});syncNav(name);root.innerHTML='';
    current=pages[name]?.();if(!current)throw new Error(`unknown page ${name}`);current.mount(root);current.render();
