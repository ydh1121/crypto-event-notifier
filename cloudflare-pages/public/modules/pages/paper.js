...
export function createPaperPage({store}){
  ...
  function selectedStrategy(all,ex){const available=executionStrategies(all,ex),requested=String(ui().paperStrategyFilter||'all').toLowerCase(),value=requested==='all'||available.includes(requested)?requested:'all';if(value!==requested)store.setUi({paperStrategyFilter:value},{scope:'paper-strategy-normalize'});return{available,value}}
  function strategyScopedRows(all,ex,value){return value==='all'?[...all]:all.filter(r=>paperStrategy(store.get(),ex,r)===value)}

  function filteredRows(){
    const ex=ui().paperExchange,all=[...rowsFor(store.get(),ex)],strategy=String(ui().paperStrategyFilter||'all').toLowerCase();
    let list=strategyScopedRows(all,ex,strategy),q=String(ui().paperSearch||'').toLowerCase(),f=ui().paperFilter;
  ...
  function strategyFilterHtml(all,ex,available,value){
    return`<section class="paper-strategy-filter"><div><b>실행 전략</b><small>현재 실행 가상매매에 실제로 있는 전략만 표시합니다. 전략 비교용 시험 전략은 섞지 않습니다.</small></div><div class="chip-row"><button data-paper-strategy="all" class="${value==='all'?'active':''}">전체 실행 전략</button>${available.map(strategy=>`<button data-paper-strategy="${esc(strategy)}" class="${value===strategy?'active':''}">${esc(strategyLabel(strategy))}</button>`).join('')}</div></section>`;
  }

  function renderCoins(){
    const box=root?.querySelector('#paperBody');
    if(!box)return;
    const ex=ui().paperExchange,all=rowsFor(store.get(),ex),strategyState=selectedStrategy(all,ex),scopeRows=strategyScopedRows(all,ex,strategyState.value),list=filteredRows(),selected=ensurePaperMarket(),counts={
      all:scopeRows.length,
      holding:scopeRows.filter(r=>r.has_position).length,
      completed:scopeRows.filter(r=>!r.has_position&&n(r.closed_trades)>0).length,
      profit:scopeRows.filter(r=>n(r.return_pct)>0).length,
      loss:scopeRows.filter(r=>n(r.return_pct)<0).length
    },strategyName=strategyState.value==='all'?'전체 실행 전략':strategyLabel(strategyState.value);
    box.innerHTML=`${strategyFilterHtml(all,ex,strategyState.available,strategyState.value)}<section class="paper-toolbar"><div>${exchangeToggle(ex,'data-paper-exchange')}</div><input data-paper-search type="search" value="${esc(ui().paperSearch)}" placeholder="코인 검색"><select data-paper-sort><option value="return_desc" ${ui().paperSort==='return_desc'?'selected':''}>수익률 높은 순</option><option value="return_asc" ${ui().paperSort==='return_asc'?'selected':''}>수익률 낮은 순</option><option value="trades_desc" ${ui().paperSort==='trades_desc'?'selected':''}>거래 많은 순</option><option value="win_desc" ${ui().paperSort==='win_desc'?'selected':''}>승률 높은 순</option><option value="position_desc" ${ui().paperSort==='position_desc'?'selected':''}>보유금액 큰 순</option><option value="opportunity_desc" ${ui().paperSort==='opportunity_desc'?'selected':''}>기회점수 높은 순</option></select><button data-paper-reset>전체 보기</button></section><div class="chip-row paper-filter"><button data-paper-filter="all" class="${ui().paperFilter==='all'?'active':''}">전체 ${counts.all}</button><button data-paper-filter="holding" class="${ui().paperFilter==='holding'?'active':''}">보유 ${counts.holding}</button><button data-paper-filter="completed" class="${ui().paperFilter==='completed'?'active':''}">매매완료 ${counts.completed}</button><button data-paper-filter="profit" class="${ui().paperFilter==='profit'?'active':''}">수익 ${counts.profit}</button><button data-paper-filter="loss" class="${ui().paperFilter==='loss'?'active':''}">손실 ${counts.loss}</button></div><section class="paper-workspace"><aside class="paper-master"><div class="paper-master-head"><span>${list.length}개 표시</span><small>${esc(strategyName)} · ${ex==='upbit'?'업비트':'빗썸'}</small></div><div id="paperList"></div></aside><section id="paperDetail" class="paper-detail"></section></section>`;
  ...
  async function renderPaperDetail(market){
    ...
    try{
      ...
      if(id===seq){
        ...
        if(d)d.innerHTML=empty('상세 가상매매 데이터를 불러오지 못했습니다.',err.message);
      }
    }
  }
  ...
  const click=e=>{
    ...
    if(ex){lastDetail=null;lastDetailKey='';store.setUi({paperExchange:ex.dataset.paperExchange,paperMarket:'',paperStrategyFilter:'all'},{scope:'paper'});render();return}
    const strategy=e.target.closest('[data-paper-strategy]');
    if(strategy){store.setUi({paperStrategyFilter:strategy.dataset.paperStrategy,paperMarket:''},{scope:'paper-strategy'});renderCoins();return}
    ...
    if(e.target.closest('[data-paper-reset]')){store.setUi({paperFilter:'all',paperStrategyFilter:'all',paperSearch:'',paperSort:'return_desc'},{scope:'paper'});renderCoins();return}
