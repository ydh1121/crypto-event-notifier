...
export function createStrategyPage({store}){
  let root=null,unsub=null,breakdownSearch='',matrixSearch='',overviewDetailTab='summary',overviewRailScroll=0;
  const detailScroll={summary:0,coins:0,evidence:0};
  ...
  function detailTabsHtml(){
    ...
    return`<nav class="strategy-v5-detail-tabs" aria-label="선택한 매매방법 보기">${item('summary','요약')}${item('coins','코인별 성과')}${item('evidence','검증 흐름')}<button type="button" class="strategy-v5-back" data-strategy-v5-back>전략 목록</button></nav>`;
  }
  ...
  function render(){
    ...
    if(!state.snapshot){
      root.innerHTML=pageHead('매매방법 비교','여러 가상 매매방법의 성적을 비교합니다.')+loading('시험 결과를 불러오는 중입니다.');
      return;
    }
    const ex=ui().strategyExchange,data=strategyLab(state),criteria=data.candidate_criteria||{},rows=baseRows();
    if(!rows.length){
      root.innerHTML=pageHead('매매방법 비교','여러 가상 매매방법의 성적을 비교합니다.',exchangeToggle(ex,'data-strategy-exchange'))+scopeBanner('strategy')+empty('시험 결과를 기다리는 중입니다.');
      return;
    }
    const tab=currentTab();
    root.innerHTML=`${pageHead('매매방법 비교','비교할 방법이나 코인을 고르면 결과가 같은 작업공간에서 바뀝니다.',exchangeToggle(ex,'data-strategy-exchange'))}<p class="strategy-context-line">각 방법은 별도 가상계좌로 시험합니다. 현재 실행 중인 모의투자 결과와는 분리되어 있습니다.</p><nav class="subnav strategy-subnav"><button data-strategy-tab="overview" class="${tab==='overview'?'active':''}">매매방법별 비교</button><button data-strategy-tab="matrix" class="${tab==='matrix'?'active':''}">코인별로 비교</button><button data-strategy-tab="paper" class="${tab==='paper'?'active':''}">현재 실행 성적</button></nav><section id="strategyBody"></section>`;
  ...
  function renderOverview(rows,criteria,ex){
    ...
    box.innerHTML=`${overviewKpis(rows,criteria,ex)}<section class="strategy-workspace" data-strategy-workspace-v5="ready"><div class="strategy-table" data-preserve-scroll><div class="strategy-v5-rail-head"><span><b>시험 중인 매매방법</b><small>다른 방법을 누르면 오른쪽 결과만 바뀝니다.</small></span><em>수익률 순</em></div>${strategyRailRowsHtml(rows,criteria,selectedKey)}</div><section class="strategy-v5-detail-shell" aria-label="선택한 매매방법 상세">${detailTabsHtml()}<div class="strategy-v5-detail-panels"><aside id="strategyDetail" class="strategy-detail strategy-v5-panel" data-strategy-v5-panel="summary"></aside><section id="strategyBreakdown" class="strategy-v5-panel" data-strategy-v5-panel="coins"></section><section id="strategyEvidence" class="strategy-v5-panel" data-strategy-v5-panel="evidence"></section></div></section></section>`;
    renderOverviewDetails(chosen,criteria);
    requestAnimationFrame(()=>{
      const list=root?.querySelector('.strategy-table');
      if(list)list.scrollTop=overviewRailScroll;
      restoreDetailScroll();
    });
  }
  ...
  function captureDetailScroll(){
    const panels=root?.querySelector('.strategy-v5-detail-panels');
    if(panels)detailScroll[normalizedDetailTab()]=panels.scrollTop;
  }

  function restoreDetailScroll(){
    const panels=root?.querySelector('.strategy-v5-detail-panels');
    if(panels)panels.scrollTop=detailScroll[normalizedDetailTab()]||0;
  }
  ...
  function applyOverviewDetailTab(){
    ...
    shell.querySelectorAll('[data-strategy-v5-panel]').forEach(panel=>{
      panel.hidden=panel.dataset.strategyV5Panel!==tab;
    });
  }
  ...
  function renderSummary(r,criteria){
    ...
    box.innerHTML=`<header><span>지금 선택한 매매방법</span><h3>${esc(r.label||r.style)}</h3><p>${esc(r.description||'설명 없음')}</p><em class="status-badge ${status}">${esc(label)}</em></header><div class="strategy-detail-kpis"><span><small>전체 수익률</small><b class="${tone(r.return_pct)}">${pct(r.return_pct)}</b></span><span><small>1,000만원 기준 평가액</small><b>${money(normalized.equity)}</b></span><span><small>최대 하락폭</small><b class="${tone(r.max_drawdown_pct)}">${pct(r.max_drawdown_pct)}</b></span><span><small>번 돈 ÷ 잃은 돈</small><b>${n(r.profit_factor)>=999?'∞':n(r.profit_factor).toFixed(2)}</b></span><span><small>완료 거래</small><b>${n(r.closed_trades)}회</b></span><span><small>승률</small><b>${n(r.win_rate_pct).toFixed(1)}%</b></span><span><small>거래당 평균 결과</small><b>${pct(r.expectancy_pct)}</b></span><span><small>수익 코인 비율</small><b>${pct(n(c.profitable_market_share)*100)}</b></span></div><p class="strategy-detail-next">코인별 결과가 궁금하면 위의 ‘코인별 성과’를 누르세요. 검증 과정은 ‘검증 흐름’에서 따로 볼 수 있습니다.</p>`;
  }
  ...
  const click=e=>{
    ...
    if(detailTab){
      ...
      if(!['summary','coins','evidence'].includes(value))return;
      captureDetailScroll();
    ...
    if(row){
      const list=root?.querySelector('.strategy-table');
      if(list)overviewRailScroll=list.scrollTop;
      captureDetailScroll();
  ...
  return{
    mount(r){root=r;root.addEventListener('click',click);root.addEventListener('input',input);root.addEventListener('change',change);unsub=store.subscribe((_,m)=>{if(m.type==='snapshot'){const list=root?.querySelector('.strategy-table');if(list)overviewRailScroll=list.scrollTop;captureDetailScroll();render()}})},
    render,
