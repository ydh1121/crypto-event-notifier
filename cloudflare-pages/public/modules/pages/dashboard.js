...
export function createDashboardPage({store,navigate}){
  ...
  function recentChanges(state){
    ...
    for(const ex of ['bithumb','upbit']){
      ...
      for(const row of Array.isArray(d.fills)?d.fills:[]){out.push({ts:n(row.ts),ex,label,market:row.market||'',kind:row.side==='sell'?'매도':'매수',text:row.side==='sell'?`실현 ${n(row.realized_pnl)>=0?'+':''}${money(row.realized_pnl)}`:`${money(row.krw)} 체결`,tone:row.side==='sell'?tone(row.realized_pnl):''})}
      for(const row of Array.isArray(d.feedback)?d.feedback:[]){out.push({ts:n(row.ts),ex,label,market:row.market||'',kind:'학습',text:`결과 ${pct(row.outcome_return_pct)} 반영`,tone:tone(row.outcome_return_pct)})}
    }
  ...
  function render(){
    ...
    const bh=paperStats(state,'bithumb'),uh=paperStats(state,'upbit'),sm=marketSummary(state,'bithumb'),lab=strategyLab(state),cs=lab.candidate_summary||{},changes=recentChanges(state);
    ...
    root.innerHTML=`${pageHead('대시보드','지금 확인할 위험·기회와 핵심 상태만 먼저 봅니다.')}
      ...
      <section class="section-panel dashboard-activity"><header><div><h3>최근 중요 변화</h3><p>두 거래소의 최근 PAPER 체결과 학습 변화를 시간순으로 합칩니다.</p></div><button data-route="records">전체 기록</button></header><div class="activity-list">${changes.length?changes.map(x=>`<div class="activity-row"><span class="activity-kind">${esc(x.kind)}</span><span><b>${esc(String(x.market).replace(/^KRW-/,'')||'-')}</b><small>${esc(x.label)}</small></span><span class="${x.tone}">${esc(x.text)}</span><time>${dt(x.ts)}</time></div>`).join(''):'<p class="muted">최근 체결·학습 변화가 아직 없습니다.</p>'}</div></section>`;
  }
