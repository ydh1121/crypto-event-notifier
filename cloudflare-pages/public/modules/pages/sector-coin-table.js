...
function sortButton(current,key,label){const active=current.startsWith(key);return`<button data-sector-sort="${key}" class="${active?'active':''}" aria-pressed="${active?'true':'false'}">${label} <i>${sectorCoinSortArrow(current,key)}</i></button>`}
...
export function renderSectorCoinTable({coins,selected,sort,turnover}){
  ...
  return`<section class="sector-coin-table"><header><div><h3>섹터 상세 코인</h3><span>${coins.length}종목 표시</span></div><small>기본 표는 핵심 기간만 표시하고, 선택한 코인에서 전체 일별·누적 구간을 펼쳐 봅니다.</small></header><div class="sector-coin-row columns"><span>코인</span>${sortButton(normalized,'return24h','24H')}${sortButton(normalized,'return7d','7D')}${sortButton(normalized,'return30d','30D')}${sortButton(normalized,'turnover','거래대금')}${sortButton(normalized,'opportunity','기회')}</div><div class="sector-coin-list">${coins.map(c=>{
