...
export function renderSectorCoinTable({coins,selected,sort,turnover}){
  ...
  return`<section class="sector-coin-table"><header><div><h3>섹터 상세 코인</h3><span>${coins.length}종목 표시</span></div><small>D-5부터 오늘까지 같은 자리에서 비교합니다.</small></header><div class="sector-coin-row columns"><span>코인</span><button data-sector-sort="change">D-5 · D-4 · D-3 · D-2 · D-1 · 24H <i>${sectorCoinSortArrow(normalized,'change')}</i></button><button data-sector-sort="turnover">24H 거래대금 <i>${sectorCoinSortArrow(normalized,'turnover')}</i></button><button data-sector-sort="opportunity">기회점수 <i>${sectorCoinSortArrow(normalized,'opportunity')}</i></button></div><div class="sector-coin-list">${coins.map(c=>{
    const lifecycle=lifecycleMeta(c.lifecycle_state);
    return`<button class="sector-coin-row sector-coin-select ${c.market===selected?'selected':''}" data-sector-market="${esc(c.market)}"><span class="sector-coin-name"><b>${esc(c.name_ko||c.symbol)}</b><small>${esc(c.name_en||c.symbol)} · <em class="${lifecycle.className}">${esc(c.symbol)}</em>${lifecycle.label?`<strong class="market-lifecycle-label ${lifecycle.className}">${esc(lifecycle.label)}</strong>`:''}</small></span><span class="sector-return-strip">${WINDOWS.map(([key,label])=>returnCell(c,key,label)).join('')}</span><span>${turnover(c.turnover_24h)}</span><span>${n(c.opportunity_score).toFixed(0)}</span></button>`
