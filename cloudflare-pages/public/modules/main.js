...
function renderShell(){const s=store.get(),user=s.user,status=document.getElementById('systemStatusBtn'),userBtn=document.getElementById('userMenuBtn');if(userBtn)userBtn.innerHTML=`<span>${esc(user?.display_name||'사용자')}</span><small>${user?.role==='owner'?'관리자':'조회'}</small>`;if(status){const ts=latestSource(s);status.textContent=s.error?'자료를 불러오지 못함':ts?`● ${age(ts)} 갱신`:'● 자료를 기다리는 중';status.className=`utility-status ${s.error?'bad':ts?'good':'neutral'}`}}
