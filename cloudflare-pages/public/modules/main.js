...
import{createAssetsPage}from'./pages/assets.js?v=48';
...
const root=document.getElementById('pageRoot'),nav=document.getElementById('mainNav'),journeyNav=document.getElementById('journeyNav');
...
store.subscribe((_,meta)=>{if(['snapshot','error','user','session-reset'].includes(meta.type))renderShell();if(meta.type==='snapshot'&&router.current()==='dashboard-detail'&&!root.querySelector('[data-dashboard-root]'))router.render()});
document.getElementById('systemStatusBtn')?.addEventListener('click',()=>router.go('system'));document.getElementById('userMenuBtn')?.addEventListener('click',()=>router.go('system'));auth.boot();
