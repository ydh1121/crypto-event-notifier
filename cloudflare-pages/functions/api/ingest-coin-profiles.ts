...
function evidenceList(value: unknown): Array<{source:string;url:string;label:string;language:string;weight:number}> {
  ...
  for (const raw of value.slice(0, 16)) {
    ...
    out.push({source, url, label: clean(item.label, 180), language: clean(item.language, 40), weight: clamp(Number(item.weight || 0), 0, 1)});
  }
  return out;
}
...
export const onRequestPost: PagesFunction<Env> = async ({request, env}) => {
  ...
  for (const raw of profiles) {
    ...
    const confidence = clamp(Number(raw.match_confidence || 0), 0, 1);
    const sourceCount = clamp(Math.round(Number(raw.source_count || evidence.length)), 0, 99);
    const verifiedAtRaw = Number(raw.verified_at || now);
    const verifiedAt = Number.isFinite(verifiedAtRaw) ? Math.round(verifiedAtRaw) : now;
  ...
  if (!statements.length) return error(422, 'NO_VALID_PROFILES', '유효한 코인 프로필이 없습니다.');
  const results = await env.DB.batch(statements);
  return json({ok: true, stored: results.length, received_at: now, research_status: statusCounts});
};
