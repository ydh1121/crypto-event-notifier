...
export const onRequestGet: PagesFunction<Env> = async ({env}) => {
  const row = await env.DB.prepare(
    `SELECT received_at,source_ts,
      (SELECT COUNT(*) FROM snapshots) AS snapshot_count,
      (SELECT MIN(received_at) FROM snapshots) AS oldest_received_at
     FROM snapshots ORDER BY id DESC LIMIT 1`,
  ).first<{received_at: number; source_ts: number; snapshot_count: number; oldest_received_at: number}>();
  ...
  return json({
    ...
    oldest_received_at: Number(row?.oldest_received_at || 0),
  });
};
