...
export function evaluateProfileIntegrity(row:ProfileIntegrityRow,official:ExchangeMarketName|undefined,index:IntegrityProjectIndex):IntegrityFinding{
  ...
  if(leadId){const identities=projectsForIdentity(index,leadId.name,leadId.alias),hasCurrent=identities.some(isCurrent),foreignIdentities=identities.filter(project=>!isCurrent(project));if(!hasCurrent&&foreignIdentities.length){reasons.push('content_lead_name_mismatch');for(const project of foreignIdentities)add(project,'content_foreign_identity')}}
  ...
  const relatedToCurrent=(project:KnownProject)=>currentAppears&&(leadNorm.includes(normName(project.english_name))||leadNorm.includes(normName(project.korean_name))||sharedDistinctiveToken(officialEn,project.english_name));
