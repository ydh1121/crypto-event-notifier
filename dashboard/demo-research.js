(function(){
  ...
  function ensureHome(){
    ...
    if(!card){
      card=document.createElement('section');card.id='autoDemoCard';card.className='panel auto-demo-card demo-home-card';
      const holdings=q('#myHoldingsOverview');(holdings||overview.querySelector('.kpi-grid'))?.insertAdjacentElement('afterend',card);
    }
    return card;
  }
  ...
  function ensureResearch(){
    ...
    root.innerHTML=`
      ...
      <div class="demo-layout">
        <div class="demo-leaderboard"><input id="demoSearch" class="demo-search" inputmode="search" placeholder="코인 검색 · BTC, XRP, B3..." autocomplete="off"><div id="demoList" class="demo-list"></div></div>
    ...
    q('#demoSearch',root)?.addEventListener('input',event=>{state.filter=event.target.value.trim().toUpperCase();renderLeaderboard()});
    return root;
  }
  ...
  function renderHome(){
    ...
    card.innerHTML=`
      <div class="panel-head"><div><h3>코인별 1,000만원 가상매매</h3><p class="panel-copy">빗썸 각 코인을 별도 1,000만원 계좌로 비교합니다.</p></div><span class="status-pill ${data.running?'good':'neutral'}">${data.running?'연구 중':'대기'}</span></div>
      <div class="demo-home-best"><span>현재 수익률 1위</span><b>${best?`${esc(best.symbol)} <span class="${tone(best.return_pct)}">${percent(best.return_pct)}</span>`:'집계 중'}</b></div>
      <div class="auto-demo-row"><span>연구 대상</span><b>${Number(data.market_count||0).toLocaleString('ko-KR')}개 코인</b></div>
      <div class="auto-demo-row"><span>현재 스캔</span><b>${Number(data.scanned_count||0).toLocaleString('ko-KR')} / ${Number(data.scan_total||0).toLocaleString('ko-KR')}</b></div>
      <div class="auto-demo-row"><span>포지션 보유</span><b>${Number(data.active_positions||0).toLocaleString('ko-KR')}개</b></div>
      <p class="demo-home-note">실제 주문은 없습니다. 결과 탭에서 코인별 체결 시점, 주문 비중, 손익과 자동 조정된 PAPER 프로필을 확인할 수 있습니다.</p>`;
  }
  ...
  function renderSummary(){
    ...
    q('#demoSummaryGrid',root).innerHTML=`
      <div class="demo-stat"><span>코인별 시작금</span><strong>1,000만원</strong><small>서로 독립된 계좌</small></div>
      <div class="demo-stat"><span>전체 대상</span><strong>${Number(data.market_count||0).toLocaleString('ko-KR')}개</strong><small>빗썸 원화마켓</small></div>
      <div class="demo-stat"><span>평균 수익률</span><strong class="${tone(avg)}">${percent(avg)}</strong><small>각 계좌 단순 평균</small></div>
      <div class="demo-stat"><span>보유 중</span><strong>${Number(data.active_positions||0).toLocaleString('ko-KR')}개</strong><small>실제 주문 없음</small></div>`;
  ...
  async function selectMarket(market){
    state.selected=market;renderLeaderboard();await loadDetail(market);
  }
  ...
  function profileChange(feedback){
    const before=feedback.profile_before||{},after=feedback.profile_after||{};
    return `<div class="demo-feedback-grid"><span>시장 ${Number(before.regime_floor||0).toFixed(1)} → <b>${Number(after.regime_floor||0).toFixed(1)}</b></span><span>진입 ${Number(before.entry_floor||0).toFixed(1)} → <b>${Number(after.entry_floor||0).toFixed(1)}</b></span><span>기본비중 ${Number(before.base_weight_pct||0).toFixed(1)}% → <b>${Number(after.base_weight_pct||0).toFixed(1)}%</b></span></div>`;
  }

  async function loadDetail(market){
    const root=q('#demoResearch'),detail=q('#demoDetail',root);if(!detail||!market)return;
    detail.innerHTML='<div class="demo-detail-empty">코인별 결과를 불러오는 중입니다.</div>';
    try{
      ...
      detail.innerHTML=`
        <div class="demo-detail-head"><div><h4>${esc(s.symbol||market.replace('KRW-',''))}</h4><p>${esc(s.name||market)} · PAPER 프로필 v${Number(s.profile_version||profile.version||1)}</p></div><strong class="demo-return ${tone(s.return_pct)}">${percent(s.return_pct)}</strong></div>
        <div class="demo-score-line"><div><span>기회 점수</span><b>${Number(signal.opportunity_score||0).toFixed(1)}</b></div><div><span>시장</span><b>${Number(signal.regime_score||0).toFixed(1)}</b></div><div><span>진입</span><b>${Number(signal.entry_score||0).toFixed(1)}</b></div></div>
        <div class="demo-profile"><div><span>현재 판단</span><b>${intentLabel(signal.trade_intent)}</b></div><div><span>권장 비중</span><b>${Number(signal.suggested_weight_pct||0).toFixed(1)}%</b></div><div><span>누적 거래</span><b>${Number(s.closed_trades||0)}회</b></div><div><span>최대 하락폭</span><b>${Number(s.max_drawdown_pct||0).toFixed(2)}%</b></div></div>
        <div class="demo-profile"><div><span>시장 기준</span><b>${Number(profile.regime_floor||0).toFixed(1)}</b></div><div><span>진입 기준</span><b>${Number(profile.entry_floor||0).toFixed(1)}</b></div><div><span>탐색 기준</span><b>${Number(profile.exploration_floor||0).toFixed(1)}</b></div><div><span>기본 비중</span><b>${Number(profile.base_weight_pct||0).toFixed(1)}%</b></div></div>
        <div class="demo-chart ${tone(s.return_pct)}">${equitySvg(history)}</div>
        <div class="demo-section-title"><h5>매매 내역</h5><small>시점 · 비중 · 결과</small></div>
        <div class="demo-trades">${fills.length?fills.slice(0,30).map(row=>`<div class="demo-trade"><span>${time(row.ts)}</span><b class="${row.side}">${sideLabel(row.side)}</b><span>${won(row.krw)}<br><small>${Number(row.weight_pct||0).toFixed(1)}%</small></span><span class="${tone(row.return_pct)}">${row.side==='sell'?percent(row.return_pct):won(row.price)}</span><small class="demo-trade-reason" title="${esc(row.reason)}">${esc(row.reason)}</small></div>`).join(''):'<div class="demo-detail-empty">아직 체결된 가상매매가 없습니다.</div>'}</div>
        <div class="demo-section-title"><h5>자동 개선 기록</h5><small>종료된 거래의 피드백</small></div>
    ...
    }catch(err){detail.innerHTML='<div class="demo-detail-empty">첫 스캔이 아직 이 코인까지 도달하지 않았습니다. 전체 스캔이 진행되면 자동으로 생성됩니다.</div>'}
  }
  ...
  async function refresh(){
    ...
    try{
      ...
      const card=ensureHome();if(card)card.innerHTML='<p class="muted">가상매매 엔진이 최신 런타임으로 올라오기를 기다리고 있습니다.</p>';
    }
  }
