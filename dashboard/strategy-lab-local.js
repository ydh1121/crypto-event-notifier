(()=>{
  ...
  function candidateMeta(row){const c=row?.candidate||{},criteria=state?.candidate_criteria||{};if(row?.status==='paused'||c.status==='paused')return['검증 중지','neutral'];if(c.status==='candidate')return['후보 통과','good'];if(c.status==='rejected')return['게이트 미통과','bad'];return[`검증 ${n(c.closed_trades||row?.closed_trades).toFixed(0)}/${n(criteria.min_closed_trades)||30}`,'warn']}

  function render(){
    const panel=ensurePanel();if(!panel||!state)return;
    const custom=Array.isArray(state.custom)?state.custom:[],can=Boolean(state.can_control),baseA=builtins()[1]?.style||builtins()[0]?.style||'balanced',baseB=builtins()[3]?.style||builtins()[0]?.style||'dca',spec=selectedSpec(baseA),summary=state.candidate_summary||{},criteria=state.candidate_criteria||{};
    panel.innerHTML=`<div class="strategy-lab-local-head"><div><p class="panel-kicker">PHASE 4 · LOCAL ONLY</p><h3>조합형 전략 실험</h3><p>두 기본전략을 섞어 별도 PAPER 실험을 만듭니다. 실제 생성·중지는 이 PC에서만 가능하고 외부 Pages는 결과만 조회합니다.</p></div><span class="status-pill ${can?'good':'neutral'}">${can?'이 PC에서 생성 가능':'조회만 가능'}</span></div><div class="strategy-lab-candidate-summary"><span>후보 통과 <b>${n(summary.candidate).toFixed(0)}</b></span><span>검증 중 <b>${n(summary.warming).toFixed(0)}</b></span><span>게이트 미통과 <b>${n(summary.rejected).toFixed(0)}</b></span><span>최소 표본 <b>${n(criteria.min_closed_trades)||30}회 / ${n(criteria.min_traded_markets)||5}종목</b></span></div><div class="strategy-lab-local-grid"><section class="strategy-lab-builder"><h4>새 실험 만들기</h4><form id="strategyLabLocalForm"><div class="strategy-lab-form-grid"><label class="wide">실험 이름<input id="labCustomLabel" maxlength="48" placeholder="예: 역추세 + 분할매수 60/40" required></label><label>거래소<select id="labCustomExchange"><option value="bithumb">빗썸</option><option value="upbit">업비트</option></select></label><label>주 전략<select id="labPrimary">${styleOptions(baseA)}</select></label><label>보조 전략<select id="labSecondary">${styleOptions(baseB)}</select></label><label>주 전략 비중<div class="strategy-lab-mix"><input id="labMix" type="range" min="0" max="100" value="60"><output id="labMixOutput">60%</output></div></label><details class="strategy-lab-advanced"><summary>세부 기준 직접 덮어쓰기</summary><div id="labAdvancedGrid" class="strategy-lab-advanced-grid">${advancedFields(spec)}</div></details></div><div class="strategy-lab-builder-actions"><button class="button" type="submit" ${can?'':'disabled'}>연구 시작</button><span id="labCreateStatus" class="muted"></span></div><p class="strategy-lab-builder-note">최대 사용자 실험은 전체 12개, 거래소별 6개입니다. 새 실험은 기존 adaptive 및 기본 12개 전략 계좌와 완전히 분리됩니다. 후보 통과도 실제 주문 전략으로 자동 승격되지 않습니다.</p></form></section><section class="strategy-lab-custom-list"><h4>사용자 실험</h4><div class="strategy-lab-custom-list-body">${custom.length?custom.map(r=>{const [candidateLabel,candidateTone]=candidateMeta(r),c=r.candidate||{};return`<article class="strategy-lab-custom-row" data-exp="${esc(r.experiment_id)}"><div><h5>${esc(r.label)}</h5><p>${r.exchange==='upbit'?'업비트':'빗썸'} · ${esc(selectedSpec(r.primary_style)?.label||r.primary_style)} ${(n(r.mix_ratio)*100).toFixed(0)}% + ${esc(selectedSpec(r.secondary_style)?.label||r.secondary_style)} ${(100-n(r.mix_ratio)*100).toFixed(0)}%</p><div class="metrics"><span>수익 <b class="${tone(r.return_pct)}">${pct(r.return_pct)}</b></span><span>DD <b>${pct(r.max_drawdown_pct)}</b></span><span>완료 <b>${n(r.closed_trades).toFixed(0)}</b></span><span>거래 종목 <b>${n(c.traded_markets).toFixed(0)}</b></span><span>게이트 <b>${n(c.passed_gates).toFixed(0)}/${n(c.total_gates).toFixed(0)}</b></span></div></div><div class="strategy-lab-custom-row-actions"><span class="status-pill ${candidateTone}">${candidateLabel}</span><span class="status-pill ${r.status==='running'?'good':'neutral'}">${r.status==='running'?'실행 중':'일시정지'}</span><button type="button" class="button secondary compact" data-lab-status="${r.status==='running'?'paused':'running'}" ${can?'':'disabled'}>${r.status==='running'?'일시정지':'재개'}</button></div></article>`}).join(''):`<div class="strategy-lab-local-empty">아직 사용자 조합 실험이 없습니다.</div>`}</div></section></div>`;
    bindForm();
  }
