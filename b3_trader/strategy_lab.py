...
from .strategy_lab_rules import STYLE_SPECS, StyleSpec, _num, entry_reason, buy_budget, style_intent, FEE_RATE, SLIPPAGE_RATE
...
class StrategyLabStore:
    ...
    def _entry_reason(spec: StyleSpec, row: dict[str, Any], learning: dict[str, Any]) -> str:
        return entry_reason(spec, row, learning)
    ...
    def _buy(
        ...
        *,
        ...
        if price <= 0:
            return None
        order_krw = buy_budget(account, learning, spec, price, START_KRW)
    ...
    def _process_style(
        ...
        *,
        ...
        intent = style_intent(spec, row, account, learning)
        if not intent:
            return None
        side, reason = intent
        if side == "sell":
            return self._sell(account=account, learning=learning, row=row, reason=reason)
        return self._buy(account=account, learning=learning, spec=spec, row=row, reason=reason)
