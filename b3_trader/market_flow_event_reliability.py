...
INTERPRETATION = (
    "overlap_clustered_spread_only_event_reliability_"
    "not_full_transaction_cost_not_probability_not_trading_score"
)
...
class MarketFlowEventReliabilityStore:
    """Reliability view over overlap-clustered, spread-adjusted market events.

    Event clusters reduce pseudo-replication before this layer evaluates edge
    consistency. The primary event hit rate uses one equal-weight market event as
    one observation. Cross-exchange confirmation is evaluated more strictly: a
    confirmed event counts as a positive agreement only when both Bithumb and
    Upbit representatives have positive spread-adjusted directional returns.

    This is still an incomplete transaction-cost view because fee and historical
    depth-ladder slippage models do not exist. It is research evidence only and is
    not wired to score, PAPER decisions, strategy mutation, or orders.
    """
    ...
    def _ensure_schema(self) -> None:
        self.conn.executescript(
            """
            CREATE TABLE IF NOT EXISTS research_market_flow_event_reliability_mx(
                ...
                source TEXT NOT NULL DEFAULT 'market_flow_event_cluster',
                received_at REAL NOT NULL,
                feature_version INTEGER NOT NULL DEFAULT 1,
                schema_version INTEGER NOT NULL DEFAULT 1,
                PRIMARY KEY(market,regime_label,horizon_label)
            );
            CREATE INDEX IF NOT EXISTS idx_market_flow_event_reliability_status
            ON research_market_flow_event_reliability_mx(
    ...
    def _event_rows(self) -> list[dict[str, Any]]:
        exists = self.conn.execute(
            "SELECT 1 FROM sqlite_master WHERE type='table' AND name='research_market_flow_event_cluster_mx'"
        ).fetchone()
        if not exists:
            return []
        rows = self.conn.execute(
            """SELECT event_id,market,regime_label,horizon_label,
                      mean_spread_adjusted_hypothesis_return_pct,cross_exchange_confirmed
               FROM research_market_flow_event_cluster_mx
               WHERE mean_spread_adjusted_hypothesis_return_pct IS NOT NULL
    ...
    def _representative_returns(self) -> dict[str, dict[str, float]]:
        exists = self.conn.execute(
            "SELECT 1 FROM sqlite_master WHERE type='table' AND name='research_market_flow_event_cluster_member_mx'"
        ).fetchone()
        if not exists:
            return {}
        result: dict[str, dict[str, float]] = defaultdict(dict)
        rows = self.conn.execute(
            """SELECT event_id,exchange,spread_adjusted_hypothesis_return_pct
               FROM research_market_flow_event_cluster_member_mx
        ...
        for row in rows:
            result[str(row["event_id"])][str(row["exchange"])] = float(
                row["spread_adjusted_hypothesis_return_pct"]
            )
        return dict(result)
    ...
    def compute(self, *, now: float | None = None) -> dict[str, Any]:
        ...
        for event in events:
            grouped[(
                str(event["market"]),
                str(event["regime_label"]),
                str(event["horizon_label"]),
            )].append(event)

        self.conn.execute("DELETE FROM research_market_flow_event_reliability_mx")
        ...
        for key, rows in grouped.items():
            event_returns = [
                float(row["mean_spread_adjusted_hypothesis_return_pct"])
                for row in rows
            ]
            ...
            for row in rows:
                ...
                by_exchange = representatives.get(event_id, {})
            ...
            if promotion_ready:
                ...
                status = "mixed_event_edge"
            else:
                status = "collecting"

            self.conn.execute(
                """INSERT INTO research_market_flow_event_reliability_mx(
                       market,regime_label,horizon_label,event_count,positive_event_count,
                       mean_event_spread_adjusted_return_pct,event_hit_rate_pct,event_wilson_lower_pct,
        ...
        return {
            "ok": True,
            "status": "computed" if grouped else "waiting_for_clustered_events",
    ...
    def audit(self) -> dict[str, Any]:
        exists = bool(self.conn.execute(
            "SELECT 1 FROM sqlite_master WHERE type='table' AND name='research_market_flow_event_reliability_mx'"
        ).fetchone())
        if not exists:
            return {
        ...
        row_count = int(self.conn.execute(
            "SELECT COUNT(*) FROM research_market_flow_event_reliability_mx"
        ).fetchone()[0])
        observation_ready_rows = int(self.conn.execute(
            "SELECT COUNT(*) FROM research_market_flow_event_reliability_mx WHERE observation_ready=1"
        ).fetchone()[0])
        promotion_ready_rows = int(self.conn.execute(
            "SELECT COUNT(*) FROM research_market_flow_event_reliability_mx WHERE promotion_ready=1"
        ).fetchone()[0])
        promotion_violations = int(self.conn.execute(
            """SELECT COUNT(*) FROM research_market_flow_event_reliability_mx
               WHERE promotion_ready=1 AND (
                   event_count<? OR cross_exchange_event_count<?
                   OR COALESCE(mean_event_spread_adjusted_return_pct,0)<=0
            ...
            (
                PROMOTION_MIN_EVENTS,PROMOTION_MIN_CROSS_EXCHANGE_EVENTS,
                PROMOTION_EVENT_WILSON_LOWER_PCT,PROMOTION_CROSS_POSITIVE_WILSON_LOWER_PCT,
            ),
        ).fetchone()[0])
        observation_violations = int(self.conn.execute(
            """SELECT COUNT(*) FROM research_market_flow_event_reliability_mx
               WHERE observation_ready=1 AND (
                   event_count<? OR cross_exchange_event_count<?
               )""",
        ...
        status_counts = {
            ...
            for row in self.conn.execute(
                "SELECT status,COUNT(*) AS n FROM research_market_flow_event_reliability_mx GROUP BY status"
            ).fetchall()
        }
        rows = [dict(row) for row in self.conn.execute(
            """SELECT * FROM research_market_flow_event_reliability_mx
               ORDER BY promotion_ready DESC,direction_consistent DESC,observation_ready DESC,
                        event_count DESC,market,regime_label,horizon_label"""
        ).fetchall()]
        columns = {
            str(row[1]) for row in self.conn.execute(
                "PRAGMA table_info(research_market_flow_event_reliability_mx)"
            ).fetchall()
        }
        suspicious_columns = sorted(
            column for column in columns
            if "trade_intent" in column.lower()
            or "order_qty" in column.lower()
            or "position_size" in column.lower()
            or "strategy_action" in column.lower()
        )
        ok = (
            promotion_violations == 0
            and observation_violations == 0
            and direction_violations == 0
            and not suspicious_columns
        )
        return {
            "ok": ok,
            "status": "ready" if row_count else "waiting_for_clustered_events",
            "table_exists": True,
            "row_count": row_count,
            "observation_ready_rows": observation_ready_rows,
            "promotion_ready_rows": promotion_ready_rows,
