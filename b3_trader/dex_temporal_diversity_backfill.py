...
STATE_PATH = Path("b3_trader/data/research-platform/dex-temporal-diversity-build57-state.json")
DEFAULT_MAX_CASES_PER_RUN = 2
MAX_CASES_PER_RUN = 2
RETRY_AFTER_SECONDS = 6 * 3600
PRIMARY_HISTORY_CUTOFF_MONTH = "2026-07"
FALLBACK_MONTH = "2026-07"
...
class DexTemporalDiversityBackfillRunner:
    """Bounded Build57 remediation for temporal spread and sample diversity.

    Only exact verified CoinGecko identities that are not already represented in
    the usable DEX sample are eligible. Actual DEX research is delegated to the
    existing Build42 cycle. Build57 intentionally reserves July as a read-only
    fallback preview and executes only June-or-earlier historical cases so that
    diversity work also reduces Build53 temporal concentration.
    """
    ...
    def plan(self, *, now: float | None = None, limit: int = 20) -> dict[str, Any]:
        ...
        july_fallback: list[dict[str, Any]] = []
        scheduled_ids: set[str] = set()
        skipped_duplicate_provider_ids = 0
        ...
        for row in self._listing_rows():
            ...
            if provider != "coingecko" or not provider_id:
                continue
            if provider_id in usable_ids:
                continue
            if provider_id in scheduled_ids:
                skipped_duplicate_provider_ids += 1
                continue
            ...
            candidate = {
                **row,
                ...
                "priority": "historical_new_unique_asset",
                "existing_usable_asset": False,
            }
            if month < PRIMARY_HISTORY_CUTOFF_MONTH:
                primary.append(candidate)
            elif month == FALLBACK_MONTH:
                july_fallback.append({**candidate, "priority": "july_unique_fallback_not_selected"})

        primary.sort(key=lambda row: (str(row.get("listing_month") or ""), float(row.get("domestic_open_at") or 0.0), str(row.get("case_key") or "")))
        july_fallback.sort(key=lambda row: (float(row.get("domestic_open_at") or 0.0), str(row.get("case_key") or "")))
        ...
        target_blocked = any(
            ...
            for prefix in (
                "unique_asset_ratio_below_min:",
                "duplicate_event_share_above_max:",
                "temporal_concentration_above_max:",
            )
        )
        ...
        elif target_blocked and primary:
            action = "temporal_diversity_backfill"
        elif target_blocked:
            action = "historical_expansion_needed"
        else:
            action = "temporal_diversity_gate_clear"
        ...
        return {
            ...
            "busy": busy,
            "policy": {
                "verified_coingecko_identity_only": True,
                "new_unique_asset_only": True,
                "one_event_per_provider_id": True,
                "primary_month_before": PRIMARY_HISTORY_CUTOFF_MONTH,
                "july_fallback_execution_enabled": False,
                "dominant_month_excluded": True,
            },
            ...
            "candidates": primary[:preview_limit],
            "july_fallback_count": len(july_fallback),
            "july_fallback": july_fallback[:preview_limit],
            "skipped_duplicate_provider_ids": skipped_duplicate_provider_ids,
            "sample_composition": {
    ...
    def run_once(self, *, max_cases: int = DEFAULT_MAX_CASES_PER_RUN) -> dict[str, Any]:
        ...
        if action != "temporal_diversity_backfill":
            return {
                "status": action,
                "paper_only": True,
                "shadow_only": True,
            ...
            results.append(
                {
                    **result,
                    ...
                    "temporal_diversity_priority": True,
                }
            )
        ...
        return {
            ...
            "before_duplicate_event_cases": int(before_events.get("duplicate_event_cases") or 0),
            "after_duplicate_event_cases": int(after_events.get("duplicate_event_cases") or 0),
